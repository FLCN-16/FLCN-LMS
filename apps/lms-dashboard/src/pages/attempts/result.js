    ...
    return (<>
      ...
      <div className="px-4 lg:px-6">
        ...
        {isLoading ? (<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          </div>) : !result ? (<p className="text-sm text-muted-foreground">Result not found.</p>) : (<div className="flex flex-col gap-6">
            {/* Score summary */}
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-2xl font-bold">
                    {result.rank ?? "—"}
                    {result.percentile != null && (<span className="ml-2 text-sm font-normal text-muted-foreground">
            ...
            {/* Section breakdown */}
            {Object.keys(result.sectionBreakdown).length > 0 && (<Card>
                <CardHeader>
                  <CardTitle className="text-base">Section Breakdown</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="divide-y">
                    {Object.entries(result.sectionBreakdown).map(([section, data]) => (<div key={section} className="flex items-center justify-between py-2 text-sm">
            ...
            {/* Topic breakdown */}
            {Object.keys(result.topicBreakdown).length > 0 && (<Card>
                <CardHeader>
                  <CardTitle className="text-base">Topic Breakdown</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="divide-y">
                    {Object.entries(result.topicBreakdown).map(([topic, data]) => (<div key={topic} className="flex items-center justify-between py-2 text-sm">
