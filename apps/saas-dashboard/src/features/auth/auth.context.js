...
function AuthProvider({ children }) {
    const [hasToken, setHasToken] = React.useState(Boolean(Cookies.get(AUTH_COOKIE_NAME)));
    const { data: sessionUser, isLoading: isLoadingSession, refetch: revalidateSession, } = useSession({
        enabled: hasToken,
        refetchOnMount: true,
    ...
    async function login(email, password, remember) {
        ...
        // Update local state to enable session query
        setHasToken(true);
    ...
    async function logout() {
        ...
        // Update local state to disable session query
        setHasToken(false);
        await revalidateSession();
    }
    const user = React.useMemo(() => {
        ...
        return {
            ...sessionUser,
            permissions: normalizePermissions(sessionUser.role, sessionUser.permissions),
        };
    }, [sessionUser]);
