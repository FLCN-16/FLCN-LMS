...
describe('ApiKeysService', () => {
  ...
  let instituteRepository: Repository<Institute>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ApiKeysService,
        {
          provide: getRepositoryToken(ApiKey, 'master'),
          useValue: mockApiKeyRepository,
        },
        {
          provide: getRepositoryToken(Institute, 'master'),
    ...
    apiKeyRepository = module.get(getRepositoryToken(ApiKey, 'master'));
    instituteRepository = module.get(getRepositoryToken(Institute, 'master'));
  });
  ...
  describe('create', () => {
    ...
    it('should throw NotFoundException if institute does not exist', async () => {
      ...
      const createDto: CreateApiKeyDto = { name: 'Test Key' };

      mockInstituteRepository.findOne.mockResolvedValue(null);

      await expect(service.create(instituteId, createDto)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException if key not found', async () => {
      ...
      await expect(
        service.findOne('inst-123', 'non-existent'),
  ...
  describe('delete', () => {
    ...
    it('should throw NotFoundException if key not found', async () => {
      ...
      await expect(service.delete('inst-123', 'non-existent')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
