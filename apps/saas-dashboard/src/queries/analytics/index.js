...
export const analytics = router("analytics", {
    overview: router.query({
        fetcher: async () => {
            const response = await fetch.get("/v1/analytics/overview");
            return response.data;
        },
    }),
    courseStats: router.query({
        fetcher: async (variables) => {
            const response = await fetch.get(`/v1/analytics/courses/${variables.id}`);
            return response.data;
        },
    }),
    testStats: router.query({
        fetcher: async (variables) => {
            const response = await fetch.get(`/v1/analytics/tests/${variables.id}`);
            return response.data;
        },
    }),
});
