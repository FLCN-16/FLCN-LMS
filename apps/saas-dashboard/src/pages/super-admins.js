    ...
    return (<div className="space-y-8 px-4 py-6 md:px-6 lg:px-8">
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        {isLoading ? (<div className="flex items-center justify-center py-12">
          ...
          </div>) : admins && admins.length > 0 ? (<Table>
            <TableHeader>
            ...
            <TableBody>
              {admins.map((admin) => (<TableRow key={admin.id} className="cursor-pointer transition-colors duration-200 hover:bg-muted/60">
                  <TableCell className="font-semibold text-foreground">
                    <Link to={`/super-admins/${admin.id}/edit`} className="text-amber-600 dark:text-amber-400 hover:underline transition-colors duration-150">
                      {admin.name}
                    </Link>
                  </TableCell>
