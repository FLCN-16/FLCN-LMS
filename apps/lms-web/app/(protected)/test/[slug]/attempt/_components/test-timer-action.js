...
function TestTimerAction() {
    ...
    if (isRunning) {
        return (<Button variant="outline" size="lg" className="ml-auto rounded-xs" onClick={toggleTimer}>
        <HugeiconsIcon icon={Time03Icon}/>
        <span>Pause</span>
      </Button>);
    }
    return (<Button size="lg" className="ml-auto rounded-xs" onClick={toggleTimer}>
      <HugeiconsIcon icon={Time03Icon}/>
      <span>Resume</span>
    </Button>);
}
