...
import { Card, CardAction, CardContent, CardFooter, CardHeader, CardTitle, } from "@flcn-lms/ui/components/card";
import { Item, ItemContent, ItemMedia, ItemTitle, } from "@flcn-lms/ui/components/item";
...
function QuestionAnswer({ answer, isCorrect }) {
    const isSelected = true;
    return (<Item variant="outline" size="default" className={cn("cursor-pointer rounded-sm bg-inherit", isCorrect && "bg-blue-500 text-white")}>
      <ItemMedia>
        <HugeiconsIcon icon={isSelected ? CheckSquare : Square} className="size-5"/>
      </ItemMedia>
      <ItemContent>
...
function TestQuestionCard() {
    ...
    useEffect(() => {
        document.addEventListener(TestTimePauseEvent.type, () => setBlurQuestion(true));
        document.addEventListener(TestTimeResumeEvent.type, () => setBlurQuestion(false));
        return () => {
            document.removeEventListener(TestTimePauseEvent.type, () => setBlurQuestion(true));
            document.removeEventListener(TestTimeResumeEvent.type, () => setBlurQuestion(false));
        };
    }, [blurQuestion]);
    return (<Card className={cn("rounded px-6 py-8 transition-all duration-300 select-none", { "blur-sm": blurQuestion })}>
