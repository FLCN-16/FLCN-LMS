...
export function AppLoader() {
    ...
    useEffect(() => {
        const timers = STEPS.slice(1).map(({ at, label: l }) => setTimeout(() => setLabel(l), at));
        return () => timers.forEach(clearTimeout);
    }, []);
    return (<div role="status" aria-label="Loading admin panel" className={cn("fixed inset-0 z-50 flex flex-col items-center justify-center gap-5", "bg-background transition-[opacity,visibility] duration-400", "visible opacity-100")}>
      {/* Logo */}
      ...
      <div className="flex flex-col items-center gap-3">
        {/* Ring spinner */}
        <div aria-hidden="true" className="h-6 w-6 animate-spin rounded-full border-2 border-border border-t-foreground"/>

        {/* Progress bar */}
        <div className="h-0.5 w-40 overflow-hidden rounded-full bg-muted">
          <div className={cn("h-full w-full rounded-full bg-foreground", "animate-[progress_2s_cubic-bezier(0.16,1,0.3,1)_forwards]")}/>
        </div>
