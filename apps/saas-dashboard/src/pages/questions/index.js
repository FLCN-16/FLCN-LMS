...
import { useApproveQuestion, useDeleteQuestion, useQuestionsList, } from "@/queries/questions";
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (<TableRow>
                ...
                </TableRow>) : questions.length === 0 ? (<TableRow>
                  <TableCell colSpan={8} className="py-8 text-center text-muted-foreground">
                    No questions yet.{" "}
                    <Link to="/questions/new" className="underline underline-offset-4">
                      Add your first question.
                    </Link>
                  </TableCell>
                </TableRow>) : (questions.map((q) => (<TableRow key={q.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        {!q.isApproved && (<Button size="icon" variant="ghost" className="size-7" title="Approve" onClick={() => approveMutation.mutate({ id: q.id })} disabled={approveMutation.isPending}>
                        ...
                        <Button size="icon" variant="ghost" className="size-7 text-destructive hover:text-destructive" title="Delete" onClick={() => deleteMutation.mutate({ id: q.id })} disabled={deleteMutation.isPending}>
