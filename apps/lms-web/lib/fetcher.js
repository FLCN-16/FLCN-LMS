 ...
 */
async function injectAuthToken(headers) {
...
export async function fetcher(url, config = {}) {
    ...
    try {
        ...
        throw new NetworkError(`Network error fetching ${url}: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
    // Parse response body
    let responseData;
    try {
        ...
        throw new ParseError(`Failed to parse JSON response from ${url}: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
    ...
    if (!response.ok) {
        ...
        const error = getErrorByStatus(response.status, response.statusText, errorMessage);
 ...
 * ```
 */
export async function fetcherGet(url, config) {
    return fetcher(url, { ...config, method: "GET" });
}
/**
 ...
 * ```
 */
export async function fetcherPost(url, config) {
    return fetcher(url, { ...config, method: "POST" });
}
/**
 ...
 * ```
 */
export async function fetcherPatch(url, config) {
    return fetcher(url, { ...config, method: "PATCH" });
}
/**
 * Convenience wrapper for PUT requests
 */
export async function fetcherPut(url, config) {
    return fetcher(url, { ...config, method: "PUT" });
}
/**
 ...
 * ```
 */
export async function fetcherDelete(url, config) {
    return fetcher(url, { ...config, method: "DELETE" });
}
