    ...
    var ApiKeyGuard = class {
        ...
        async canActivate(context) {
            ...
            if (!validationResult.isValid) {
                throw new common_1.UnauthorizedException(validationResult.message);
            }
            // Check required scopes if specified
            const requiredScopes = this.reflector.get('requiredScopes', context.getHandler());
            if (requiredScopes && requiredScopes.length > 0) {
                const hasAllScopes = requiredScopes.every((scope) => validationResult.scopes?.includes(scope));
                if (!hasAllScopes) {
                    throw new common_1.ForbiddenException(`API key missing required scopes: ${requiredScopes.join(', ')}`);
                }
            }
