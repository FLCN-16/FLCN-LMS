    ...
    try {
        ...
        <div className="mx-auto max-w-7xl px-4 py-16">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {testSeries.map((series) => (<Link key={series.id} href={`/test-series/${series.id}`} className="group">
                <div className="h-full overflow-hidden rounded-lg border bg-card transition-shadow hover:shadow-lg">
                  ...
                  <div className="flex aspect-video items-center justify-center bg-muted transition-colors group-hover:bg-muted/80">
                    {series.thumbnail ? (<img src={series.thumbnail} alt={series.title} className="h-full w-full object-cover"/>) : (<div className="text-center text-muted-foreground">
                  ...
                  {/* Content */}
                  <div className="p-6">
                    {series.category && (<p className="mb-2 text-sm text-primary">
                        {series.category}
                      </p>)}
