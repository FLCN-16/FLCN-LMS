...
function CoursePackageFeature({ title, isIncluded, }) {
    return (<div className="flex items-center-safe gap-x-1">
      <HugeiconsIcon icon={isIncluded ? Check : Cancel} size={16}/>
      <Text className="text-sm">{title}</Text>
    </div>);
}
function CoursePackage({ title, price, validityMonths, features, }) {
    return (<div className="flex cursor-pointer flex-col overflow-hidden rounded">
      <div className={cn("flex items-center justify-between px-3 py-1.5 font-mono", "bg-linear-to-r from-violet-700 to-orange-700 text-primary-foreground")}>
      ...
      <div className="flex flex-col gap-y-3 rounded-b border border-t-0 px-3 py-4">
        ...
        <div className="flex flex-col gap-y-1 rounded-b">
          {features.map((feature) => (<CoursePackageFeature key={feature.title} title={feature.title} isIncluded={feature.isIncluded}/>))}
        </div>
      </div>
    </div>);
}
