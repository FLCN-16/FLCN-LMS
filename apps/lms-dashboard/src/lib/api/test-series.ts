...
export const testSeriesApi = {
  list: () => fetch.get<TestSeries[]>("/api/test-series").then((r) => r.data),
  ...
  publish: (id: string) =>
    fetch
      .patch<TestSeries>(`/api/test-series/${id}/publish`)
      .then((r) => r.data),

  remove: (id: string) => fetch.delete(`/api/test-series/${id}`),
  ...
  getTest: (seriesId: string, testId: string) =>
    fetch
      .get<Test>(`/api/test-series/${seriesId}/tests/${testId}`)
      .then((r) => r.data),

  createTest: (seriesId: string, data: CreateTestPayload) =>
    fetch
      .post<Test>(`/api/test-series/${seriesId}/tests`, data)
      .then((r) => r.data),

  updateTest: (seriesId: string, testId: string, data: CreateTestPayload) =>
    fetch
      .patch<Test>(`/api/test-series/${seriesId}/tests/${testId}`, data)
      .then((r) => r.data),

  publishTest: (seriesId: string, testId: string) =>
    fetch
      .patch<Test>(`/api/test-series/${seriesId}/tests/${testId}/publish`)
      .then((r) => r.data),
}
