...
function LicenseDetailPage() {
    ...
    const handleSuspend = async () => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : "Failed to suspend license";
            toast.error(message);
        }
    };
    const handleReactivate = async () => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : "Failed to reactivate license";
            toast.error(message);
        }
    };
    if (isLoading) {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            ...
            {license.features && license.features.length > 0 ? (<div className="space-y-3">
                {license.features.map((feature) => (<div key={feature.name} className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-foreground">{feature.name}</p>
                      {feature.limit && (<p className="text-xs text-muted-foreground">
                          Limit: {feature.limit}
                        </p>)}
                    </div>
                    <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${feature.enabled
                    ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300"
                    : "bg-slate-100 text-slate-800 dark:bg-slate-900/30 dark:text-slate-300"}`}>
                      {feature.enabled ? "Enabled" : "Disabled"}
                    </span>
                  </div>))}
              </div>) : (<p className="text-sm text-muted-foreground">No features configured</p>)}
          </div>
        </div>
        ...
        <div className="flex gap-3 flex-col sm:flex-row">
          ...
          {license.status === "active" ? (<Button variant="outline" onClick={handleSuspend} disabled={suspendMutation.isPending} className="border-amber-200 text-amber-700 hover:bg-amber-50 dark:border-amber-800 dark:text-amber-300 dark:hover:bg-amber-950/20">
              {suspendMutation.isPending ? "Suspending..." : "Suspend License"}
            </Button>) : license.status === "suspended" ? (<Button variant="outline" onClick={handleReactivate} disabled={reactivateMutation.isPending} className="border-emerald-200 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-300 dark:hover:bg-emerald-950/20">
