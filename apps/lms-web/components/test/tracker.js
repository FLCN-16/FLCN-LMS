"use client";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger, } from "@flcn-lms/ui/components/alert-dialog";
import { Button } from "@flcn-lms/ui/components/button";
import { Card, CardContent, CardFooter, CardHeader, } from "@flcn-lms/ui/components/card";
...
(function (QuestionStatus) {
    ...
    QuestionStatus["ForReview"] = "for-review";
})(QuestionStatus || (QuestionStatus = {}));
function QuestionIndex({ status = QuestionStatus.Unanswered, isCurrent, children, }) {
    return (<div className={cn("flex size-14 cursor-pointer items-center justify-center rounded-xs text-xl font-semibold", {
      ...
      <CardFooter className="bg-transparent px-0">
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button variant="destructive" size="xl" className="w-full rounded-xs font-mono">
              Submit Final Test
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
