"use client";
import { Facebook01Icon, InstagramIcon, LinkedinIcon, YoutubeIcon, } from "@hugeicons/core-free-icons";
...
function Footer() {
    ...
    return (<footer className="relative h-(--footer-height) w-full border-t [--footer-height:520px]" style={{ clipPath: "polygon(0% 0, 100% 0%, 100% 100%, 0 100%)" }}>
        ...
        <div className="sticky top-[calc(100vh-var(--footer-height))] h-full overflow-y-auto">
          <div className="relative mx-auto flex size-full max-w-6xl flex-col justify-between gap-5">
            <div className="grid grid-cols-1 gap-8 px-4 pt-12 md:grid-cols-2 lg:grid-cols-4">
              <div className="w-full space-y-4">
                ...
                <p className="mt-8 text-sm text-muted-foreground md:mt-0">
                  Empower your learning with our comprehensive platform
                </p>
                <div className="flex gap-2">
                  {socialLinks.map((link, index) => (<Button asChild key={`social-${link.href}-${index}`} size="icon-sm" variant="ghost">
                      <Link href={link.href}>
              ...
              {footerLinkGroups.map((group) => (<div className="w-full" key={group.label}>
                  <div className="mb-10 md:mb-0">
                    <h3 className="text-sm uppercase">
                      {groupLabels[group.label]}
                    </h3>
                    <ul className="mt-4 space-y-2 text-sm text-muted-foreground md:text-xs lg:text-sm">
                      {group.links.map((link) => (<li key={link.title}>
                          <a className="inline-flex items-center hover:text-foreground [&_svg]:me-1 [&_svg]:size-4" href={link.href}>
                            {link.icon}
                            {link.title}
                          </a>
                        </li>))}
            ...
            <div className="flex flex-col items-center justify-between gap-2 border-t p-4 text-sm text-muted-foreground md:flex-row">
              <p>&copy; {currentYear} FLCN LMS. All rights reserved.</p>
              <div className="space-x-2">
                <Link className="hover:text-foreground" href="/legal/privacy-policy">
                  Privacy Policy
                </Link>
                <Link className="hover:text-foreground" href="/legal/terms-of-service">
                  Terms of Service
                </Link>
              </div>
            </div>
