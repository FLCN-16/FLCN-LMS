...
import { useCreateCustomer, useUpdateCustomer, useCustomer, } from "@/queries/customers";
    ...
    return (<div className="min-h-screen w-full bg-gradient-to-b from-background to-muted/30">
      ...
      <div className="border-b border-border bg-background/95 backdrop-blur sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4 md:px-6 lg:px-8">
          <Link to="/customers" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors duration-150">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 rounded-lg border border-border bg-card p-6 md:p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <Controller control={control} name="name" rules={{ required: "Organization name is required" }} render={({ field }) => (<Input {...field} id="name" placeholder="Physics Wallah" className={`h-11 text-base ${errors.name ? "border-destructive focus-visible:ring-destructive" : ""}`} disabled={isSubmitting}/>)}/>
              ...
              <div className="space-y-3">
                ...
                <Controller control={control} name="slug" rules={{
            required: "Slug is required",
            pattern: {
                value: /^[a-z0-9-]+$/,
                message: "Slug must be lowercase alphanumeric with hyphens only",
            },
        }} render={({ field }) => (<Input {...field} id="slug" placeholder="pw-live" className={`h-11 text-base font-mono ${errors.slug ? "border-destructive focus-visible:ring-destructive" : ""}`} disabled={isEdit || isSubmitting}/>)}/>
              ...
              <div className="space-y-3">
                ...
                <Controller control={control} name="email" render={({ field }) => (<Input {...field} id="email" type="email" placeholder="contact@example.com" className="h-11 text-base" disabled={isSubmitting}/>)}/>
              </div>

              {/* Custom Domain Field */}
              <div className="space-y-3">
                <Label htmlFor="customDomain" className="text-base font-semibold text-foreground">
                  Custom Domain (White-label)
                </Label>
                <Controller control={control} name="customDomain" render={({ field }) => (<Input {...field} id="customDomain" placeholder="learning.example.com" className="h-11 text-base" disabled={isSubmitting}/>)}/>
                <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-6">
              ...
              {/* Plan Selection */}
              <div className="space-y-3">
                <Label htmlFor="planId" className="text-base font-semibold text-foreground">
                  Subscription Plan
                </Label>
                <Controller control={control} name="planId" render={({ field }) => (<Select value={field.value || ""} onValueChange={field.onChange}>
                      <SelectTrigger id="planId" className="h-11 text-base">
                        <SelectValue placeholder="Select a plan"/>
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="">No Plan (Self-managed limits)</SelectItem>
              ...
              <div className="space-y-3">
                ...
                <Controller control={control} name="maxUsers" render={({ field }) => (<Input {...field} id="maxUsers" type="number" placeholder="100" className="h-11 text-base" disabled={isSubmitting} min="1"/>)}/>
                <p className="text-xs text-muted-foreground">
              ...
              {/* Max Courses */}
              <div className="space-y-3">
                <Label htmlFor="maxCourses" className="text-base font-semibold text-foreground">
                  Maximum Courses
                </Label>
                <Controller control={control} name="maxCourses" render={({ field }) => (<Input {...field} id="maxCourses" type="number" placeholder="50" className="h-11 text-base" disabled={isSubmitting} min="1"/>)}/>
                <p className="text-xs text-muted-foreground">
              ...
              {/* Max Storage */}
              <div className="space-y-3">
                <Label htmlFor="maxStorageGb" className="text-base font-semibold text-foreground">
                  Maximum Storage (GB)
                </Label>
                <Controller control={control} name="maxStorageGb" render={({ field }) => (<Input {...field} id="maxStorageGb" type="number" placeholder="10" className="h-11 text-base" disabled={isSubmitting} min="1"/>)}/>
                <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-6">
              ...
              {/* Logo URL */}
              <div className="space-y-3">
                <Label htmlFor="logoUrl" className="text-base font-semibold text-foreground">
                  Logo URL
                </Label>
                <Controller control={control} name="logoUrl" render={({ field }) => (<Input {...field} id="logoUrl" placeholder="https://cdn.example.com/logo.png" className="h-11 text-base" disabled={isSubmitting}/>)}/>
                <p className="text-xs text-muted-foreground">
                  URL to organization logo (recommended: 200x50px PNG/SVG)
                </p>
                {watch("logoUrl") && (<div className="mt-3 p-3 bg-muted rounded-lg">
                    <img src={watch("logoUrl")} alt="Logo preview" className="h-12 object-contain" onError={(e) => {
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                <div>
                  <Label htmlFor="isActive" className="text-base font-semibold text-foreground cursor-pointer">
                    Active
                  </Label>
                ...
                <Controller control={control} name="isActive" render={({ field }) => (<Switch id="isActive" checked={field.value ?? true} onCheckedChange={field.onChange} disabled={isSubmitting}/>)}/>
              </div>
            </div>
