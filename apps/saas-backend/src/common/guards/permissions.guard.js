...
let PermissionsGuard = (() => {
    ...
    var PermissionsGuard = class {
        ...
        canActivate(context) {
            const permission = this.reflector.getAllAndOverride(check_permission_decorator_1.CHECK_PERMISSION_KEY, [context.getHandler(), context.getClass()]);
            if (!permission) {
                return true;
            }
            ...
            this.abilityFactory.ensureRequestAccess(context, (0, casl_ability_factory_1.toAbilityUser)(request.user), permission);
            return true;
        }
    };
