...
export default async function TestHistoryPage() {
    try {
        ...
        <div className="rounded-lg border">
          {testAttempts.length === 0 ? (<div className="flex flex-col items-center justify-center py-12">
              ...
              <TableBody>
                {testAttempts.map((attempt) => (<TableRow key={attempt.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        {attempt.status === "SUBMITTED" && (<Button asChild size="sm" variant="ghost">
                            <Link href={`/panel/tests/${attempt.testId}/attempts/${attempt.id}/result`}>
                              View Result
                            </Link>
                          </Button>)}
                        {attempt.status === "IN_PROGRESS" && (<Button asChild size="sm" variant="default">
                            <Link href={`/panel/tests/${attempt.testId}/attempts/${attempt.id}`}>
                              Continue
                            </Link>
                          </Button>)}
                      </div>
