...
import { useCreateApiKey, useUpdateApiKey, } from "@/queries/api-keys";
function ApiKeyFormPage() {
    ...
    if (createdKey) {
        return (<div className="space-y-6 px-4 py-2 md:px-6 max-w-2xl">
        <Helmet>
          <title>API Key Created — FLCN SaaS Admin</title>
        </Helmet>

        <div className="flex items-center gap-4">
          <Link to="/api-keys" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
    ...
    return (<div className="space-y-6 px-4 py-2 md:px-6 max-w-2xl">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 rounded-lg border border-border p-6">
        <div className="space-y-2">
          <Label htmlFor="name">Key Name</Label>
          <Controller control={control} name="name" rules={{ required: "Name is required" }} render={({ field }) => (<Input {...field} id="name" placeholder="e.g., Production API"/>)}/>
        </div>

        <div className="space-y-3">
          <Label>Scopes</Label>
          <Controller control={control} name="scopes" render={({ field }) => (<div className="space-y-2">
                {["read", "write", "delete", "admin"].map((scope) => (<div key={scope} className="flex items-center gap-2">
                    <Checkbox id={`scope-${scope}`} checked={field.value.includes(scope)} onCheckedChange={(checked) => {
                    if (checked) {
