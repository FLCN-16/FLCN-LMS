      ...
      <div className="px-4 lg:px-6">
        <div className="mb-6 flex items-center gap-3">
          <Button variant="ghost" size="icon" asChild>
            <Link to="/panel/attempts">
              <IconArrowLeft className="size-4" />
            </Link>
          </Button>
          <div>
            <h2 className="text-xl font-semibold">Attempt Result</h2>
            <p className="font-mono text-xs text-muted-foreground">{attemptId}</p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Card>
                <CardHeader className="pb-1">
                  <CardTitle className="text-sm font-medium text-muted-foreground">Rank / Percentile</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-2xl font-bold">
                    {result.rank ?? "—"}
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-2xl font-bold">{fmt(result.timeTakenSecs)}</p>
                </CardContent>
              </Card>
            </div>
                ...
                <CardContent>
                  <div className="divide-y">
                    {Object.entries(result.sectionBreakdown).map(([section, data]) => (
                      <div key={section} className="flex items-center justify-between py-2 text-sm">
                    ...
                    {Object.entries(result.topicBreakdown).map(([topic, data]) => (
                      <div key={topic} className="flex items-center justify-between py-2 text-sm">
                        ...
                        <div className="flex gap-3 text-muted-foreground">
                          ...
                          <span className="text-destructive">-{data.wrong}</span>
                        </div>
                      </div>
                    ))}
                  </div>
