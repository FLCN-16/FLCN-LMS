...
export function NavSection({ label, items, ability, }) {
    const location = useLocation();
    const visibleItems = items
        .map((item) => ({
        ...item,
        children: item.children?.filter((child) => canAccess(ability, child.permission)) || undefined,
    }))
        .filter((item) => canAccess(ability, item.permission) &&
