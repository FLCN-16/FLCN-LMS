...
import { useTestDetail, useTestSeriesDetail, useUpdateTest, } from "@/queries/test-series";
...
export default function EditTestPage() {
    ...
    const handleSubmit = (data) => {
        ...
        mutation.mutate({ seriesId, testId, data }, {
            onSuccess: () => {
    ...
    return (<>
      ...
      <div className="px-4 lg:px-6">
        ...
        {isLoading ? (<div className="flex max-w-xl flex-col gap-4">
            ...
            <Skeleton className="h-9 w-full"/>
            <Skeleton className="h-9 w-full"/>
          </div>) : test ? (<TestForm defaultValues={test} onSubmit={handleSubmit} isLoading={mutation.isPending} submitLabel="Save Changes"/>) : (<p className="text-sm text-muted-foreground">Test not found.</p>)}
      </div>
    </>);
}
