...
export const courses = router("courses", {
    categories: {
        list: router.query({
            fetcher: async () => {
                const response = await fetch.get("/api/v1/courses/categories");
                return response.data;
            },
        }),
        byId: router.query({
            fetcher: async (variables) => {
                const response = await fetch.get(`/api/v1/courses/categories/${variables.id}`);
                return response.data;
            },
        }),
        add: router.mutation({
            mutationFn: async (variables) => {
                const response = await fetch.post("/api/v1/courses/categories", variables);
                return response.data;
            },
        }),
        update: router.mutation({
            mutationFn: async (variables) => {
                const response = await fetch.patch(`/api/v1/courses/categories/${variables.id}`, variables.data);
                return response.data;
            },
        }),
        remove: router.mutation({
            mutationFn: async (variables) => {
                await fetch.delete(`/api/v1/courses/categories/${variables.id}`);
            },
        }),
    },
    list: router.query({
        fetcher: async () => {
            const response = await fetch.get("/api/v1/courses");
            return response.data;
        },
    }),
    byId: router.query({
