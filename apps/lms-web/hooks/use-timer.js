...
import { expiryTimestamp, getSecondsFromExpiry, getTimeFromSeconds, } from "@/lib/time";
import useInterval from "./use-interval";
const DEFAULT_DELAY = 1000;
function getDelayFromExpiryTimestamp(expiryTs) {
    if (!expiryTimestamp(expiryTs))
        return null;
    const seconds = getSecondsFromExpiry(expiryTs);
    const extraMilliSeconds = Math.floor((seconds - Math.floor(seconds)) * 1000);
    return extraMilliSeconds > 0 ? extraMilliSeconds : DEFAULT_DELAY;
}
export default function useTimer({ startTimestamp, expiryTimestamp: expiry, onTimeExpire, onRangeChange, startTimestampMargin = 600, // Seconds
endTimestampMargin = 600, // Seconds
onMarginRangeChange, autoStart = true, }) {
    const [expiryTimestamp, setExpiryTimestamp] = useState(expiry);
    const [seconds, setSeconds] = useState(getSecondsFromExpiry(expiryTimestamp));
    const [isRunning, setIsRunning] = useState(autoStart);
    const [didStart, setDidStart] = useState(autoStart);
    const [isExpired, setIsExpired] = useState(getSecondsFromExpiry(expiryTimestamp) <= 0);
    ...
    useInterval(() => {
        ...
        setIsInMarginRange((prev) => {
            const current = startTimestamp - startTimestampMargin * 1000 < currentTimestamp &&
                currentTimestamp < expiryTimestamp + endTimestampMargin * 1000;
            if (prev !== current && typeof onMarginRangeChange === "function") {
                onMarginRangeChange(current, currentTimestamp, startTimestamp, expiryTimestamp);
            }
            return current;
        });
        setIsInRange((prev) => {
            const current = startTimestamp < currentTimestamp &&
                currentTimestamp < expiryTimestamp;
            if (prev !== current && typeof onRangeChange === "function") {
                onRangeChange(current, currentTimestamp, startTimestamp, expiryTimestamp);
            }
            return current;
        });
