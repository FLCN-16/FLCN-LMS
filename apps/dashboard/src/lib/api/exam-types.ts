...
import type { ExamType, CreateExamTypePayload, UpdateExamTypePayload } from '@flcn-lms/types/exam-type'
...
export const examTypesApi = {
  list: (includeInactive = false) =>
    fetch
      .get<ExamType[]>('/api/exam-types', { params: includeInactive ? { includeInactive: 'true' } : {} })
      .then((r) => r.data),
  ...
  remove: (id: string) =>
    fetch.delete(`/api/exam-types/${id}`),

  seed: () =>
    fetch.post('/api/exam-types/seed').then((r) => r.data),
}
